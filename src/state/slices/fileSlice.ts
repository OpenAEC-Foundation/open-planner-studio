...
/** Feitelijke uitkomst van de centrale opennaad (`applyOpenedImport`, en daarmee ook van
 *  `openAsDocument`); gebruikt door DevBridge en MCP voor eerlijke ids. `reusedActiveTab`: heeft de
 *  open-actie het lege, ongewijzigde actieve tabblad hergebruikt (`true`) of een nieuw document
 *  geopend (`false`)? */
...
export interface FileSlice {
   ...
   * applyLoadedProject-pad als IFC/CSV/MSPDI/MPP/P6XML. Kiest zelf geen opslagdoel en geen
   * `linkedOpen` — dat doet `openAsDocument` voor elk echt open-pad.
   */
   ...
   *  opslagdoel af (`saveTargetFor`: alleen een `canBeSaveTarget`-formaat krijgt er een) en laadt via
   *  de centrale naad `applyOpenedImport` (pristine tabblad hergebruiken of nieuw document, ook voor
   *  een meervoudige XER-import) GEKOPPELD (`linkedOpen: true`: bibliotheekbinding +
   *  herkomststempels blijven, de open-grens draait), met doorrekenen, fitten en de uur-melding.
   *  Bewust NIET hierdoor: de losse loads (`loadState`: IFCPanel, extensie-import) en
...
export const createFileSlice: AppSliceFactory<FileSlice> = (runtime) => (set, get) => {
  ...
  return {
    ...
    openAsDocument: (parsed, source) => {
      ...
      // Centrale naad (tabblad hergebruiken/nieuw, meervoudige XER-vorm, bestandsbrede melding);
      // open-pad-semantiek: identiteit + opslaan-doel zetten, GEKOPPELD laden (opslagdoel en
      // `linkedOpen` horen bij elkaar: wie naar het bronbestand terugschrijft, moet ook alles laden
      // wat erin stond), direct doorrekenen + fitten en de uur-melding evalueren.
    ...
    openFile: async (labels) => {
      try {
        ...
        // Eén open-semantiek voor elk echt open-pad (tabblad, opslagdoel, gekoppeld laden), ook
        // voor een meervoudige XER-import (`applyOpenedImport` splitst die zelf).
    ...
    openRecentFile: async (id: string, labels) => {
      ...
      try {
        ...
        // laden komen uit de ene gedeelde actie, ook voor meervoudige XER-import.
