...
function createMcpDraft(
  ...
  updateTaskFields(id: string, updates: Partial<Task>): void {
    store.setState((s) => {
      ...
      if ('calendarId' in updates && s.tasks[idx].calendarId !== calendarId) {
        ...
        if (settleCalendarChange(s.tasks[idx], s.assignments, before, s).timephasedLost) recordTimephasedLoss(id);
      }
      ...
      // Integratie #101 (valkuil a): `updates`, niet `rest` — `rest` mist `calendarId` sinds de K2-stap.
  ...
  patchTaskFields(
    ...
    store.setState((s) => {
      ...
      if ('calendarId' in top && task.calendarId !== calendarId) {
        ...
        if (settleCalendarChange(task, s.assignments, before, s).timephasedLost) recordTimephasedLoss(id);
      }
  ...
  removeResource(id: string): {
    ...
    store.setState((s) => {
      ...
      // Taaktypes-etappe (spec §5 rij 5, reviewbevinding B4) — tweeling van resourceSlice.ts's
  ...
  moveAssignment(assignmentId: string, newTaskId: string): void {
    store.setState((s) => {
      ...
      // Taaktypes-etappe (spec §5 rij 4/5, reviewbevinding B4) — tweeling van resourceSlice.ts's
      // `moveAssignment`: eraf bij de oude taak, erbij bij de nieuwe; momentopnamen vóór de wissel.
