...
interface EditableColumnConfig extends Omit<ReadonlyColumnConfig, 'copy'> {
  ...
  /** Validatiecode (`taskGrid.validation.<code>`) die uitlegt WAAROM `readOnly` hier geldt;
   *  zonder deze de generieke `readOnly`-melding. */
...
function editableColumn(config: EditableColumnConfig): TaskColumnDescriptor {
  ...
  const planWrite: Writer = (value, task, ctx) => config.readOnly?.(task, ctx)
    ? failure(config.readOnlyReason?.(task, ctx) ?? 'readOnly', value)
...
function fixedTimeColumns(): TaskColumnDescriptor[] {
  return [
    ...
    editableColumn({ id: 'task.time.remainingTime', labelKey: 'taskGrid.columns.remainingTime', category: 'progress', valueKind: 'duration', editorKind: 'duration', route: 'task-progress', ...SUMMARY_PROGRESS_READ_ONLY, read: task => task.time.remainingTime, parse: parseTaskDuration, validate: validateOptionalDuration }),
