...
export function summaryProgressOf(leaves: readonly Task[], workDaysOf: (t: Task) => number): SummaryProgress {
  ...
  for (const l of leaves) {
    ...
    // Als tekst vergeleken, zoals de datum-rollup van de fase in `applyCpmResult` dat nu doet: de
    // waarden zijn ISO-datum(tijd)en (`JJJJ-MM-DD` of `JJJJ-MM-DDTHH:mm`). Alleen een dag- en een
    // uurwaarde op dezelfde dag (gemengde kalenders) kunnen als tijdstip anders uitvallen.
    const as = l.time.actualStart;
    if (as && (actualStart === undefined || as < actualStart)) actualStart = as;
    const af = l.time.actualFinish;
    if (af && (actualFinish === undefined || af > actualFinish)) actualFinish = af;
  }
