...
export class CPMSolver {
  ...
  private forwardPass(order: string[]): Map<string, { es: Date; ef: Date }> {
    ...
    for (const taskId of order) {
      ...
      if (noPreds) {
        ...
        // Conventie C10 `p6AlapPositionedFromSuccessors`: een niet-gestarte ALAP-wortel heeft geen
        ...
        if (this.options.schedulingOptions?.p6UseTaskPlannedStartFloor === true) {
          ...
          // Conventie C10: voor een niet-gestarte ALAP-taak telt het eigen geplande venster niet.
  ...
  private applyAlap(
    ...
    // Conventie C10 `p6AlapPositionedFromSuccessors`: dezelfde ALAP-selectie (incl. Z9b-uitsluiting
  ...
  /** Conventie C10: valt deze taak onder de ALAP-positionering vanuit de opvolgers? Niet-gestart,
  ...
  /**
   * Conventie C10 `p6AlapPositionedFromSuccessors` (docblok + bron bij de sleutel in
  ...
  private applyAlapFromSuccessors(
    ...
    for (let i = alapTaskIds.length - 1; i >= 0; i--) {
      ...
      if (!task) continue;
      const cal = this.calendarFor(task);
      if (!this.isUnstartedAlapPositionedFromSuccessors(task, cal)) continue;
      const early = earlyDates.get(taskId);
      const late = lateDates.get(taskId);
      if (!early || !late) continue;
      const succs = this.successors.get(taskId) || [];
