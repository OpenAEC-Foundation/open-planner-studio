...
export class CPMSolver {
  ...
  private completedOutOfSequenceRelationSeq(predTask: Task, seq: Sequence, succCal: CalendarEngine): Sequence {
    if (!this.completedOutOfSequenceEs.has(predTask.id)) return seq;
  ...
  private forwardPass(order: string[]): Map<string, { es: Date; ef: Date }> {
    ...
    for (const taskId of order) {
      ...
      if (noPreds) {
        ...
        // Conventie C9 `p6AlapPositionedFromSuccessors`: een niet-gestarte ALAP-wortel heeft geen
        ...
        for (const seq of preds) {
          ...
          const predResult = this.completedPredecessorRelationWindow(predTask, rawPredResult);
          const relSeq = this.completedOutOfSequenceRelationSeq(predTask, seq, cal);
  ...
  /** Conventie C9: valt deze taak onder de ALAP-positionering vanuit de opvolgers? Niet-gestart,
  ...
  /**
   * Conventie C9 `p6AlapPositionedFromSuccessors` (docblok + bron bij de sleutel in
