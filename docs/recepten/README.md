# Recepten

Stap-voor-stapgidsen voor zeven terugkerende ontwikkelklussen in deze repo — waar de compiler en de
tests je tegenhouden, en waar niet. Dit zijn geen vervangingen van de code of van `CLAUDE.md`: loopt
een recept ooit achter, dan heeft de code gelijk.

Dit is repo-documentatie (Nederlands, net als de rest van de codebase — zie *Working language* in
`CLAUDE.md`), geen gebruikersdocumentatie: er is geen vertaalplicht en deze map valt buiten
`verify:docs`' artikelvalidatie (die controleert alleen `public/docs/`, de in-app-gidsen).

- [`mcp-tool.md`](mcp-tool.md) — een nieuwe `planner_*`-MCP-tool toevoegen.
- [`instelling.md`](instelling.md) — een nieuwe instelling toevoegen.
- [`i18n-sleutel.md`](i18n-sleutel.md) — een nieuwe vertaalsleutel toevoegen.
- [`ribbontabblad.md`](ribbontabblad.md) — een nieuw ribbontabblad toevoegen.
- [`in-app-gids.md`](in-app-gids.md) — een nieuwe in-app gids (`public/docs/`) toevoegen.
- [`tekstgrootte.md`](tekstgrootte.md) — een tekstgrootte kiezen uit de zes rollen, of er één toevoegen.
- [`conventie.md`](conventie.md) — een nieuwe rekenconventie (rekenprofielen) toevoegen.
