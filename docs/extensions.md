...
- `api.data.addTask` met een urentaak (`time.durationUnit: 'hours'` of `time.durationMinutes`) zonder `time.scheduleFinish`: de app leidt het geplande einde af uit start + duur op de taakkalender, net als bij een taak die de gebruiker toevoegt. Geef je zelf een `scheduleFinish` mee, dan wint dat. `sdk.factory.createTask` leidt niets af (geen document, geen kalender).
