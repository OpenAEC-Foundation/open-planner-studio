...
| baan C: M1.3–M1.5 + C1–C9 | `claude/rekenprofielen-baan-c` | klaar (`dcbb0f6a`, gepusht): p6Source weg, solveInput, pset bedraad (+ overrides letterlijk), testmigratie, C5 gedragswijziging, melding/actieveld, export-guard, ext-API 1.2.0, MCP; X12 15.056 + detailrapport byte-identiek; critreview loopt. Merge in etappebranch: één conflict in `check-xer-product-fidelity-x12.ts` (C vs cellen) |
