...
| X12 naar nul — brok 4: B08 FF→startmijlpaal (C6) + B05 actief-rest-0 late kant (C7) | `claude/x12-brok4-ff-mijlpaal` (basis `1aa63a40` = C4; worktree `agent-x12-brok4-ff-mijlpaal`) | **klaar, kop `5fcd85de` (gepusht), critreview = LANDEN-MET-FIXES (C6: drijvend FF→startmijlpaal verschuift ES ongemeten; voortgangstak zonder bron; "(gemeten)" bij uurpoort; C7 trace-kleinigheid) — landfixes klaar `094b0d2e`; etappebranch (brok 2 definitief) ingemerged en 6-stappen-herpin gedaan: **kop `cf6fdef5` (gepusht)**, meting gelijk 11.529 (P6-doorgerekend 1.274 / overig 10.252 / onbekend 3), blast-radius en task-replay-pin herpind, telling eenentwintig overal, hernummerd naar **C7/C8**; `LEGACY_XER_ALSO_ON_X12` bevat nu C1–C4 + C7/C8 (brok 3 doet hetzelfde met C1–C6). Wacht op de critreview van brok 3 (C4) en merget daarna**: C7 `p6FinishFinishStartMilestoneLateFinish` (B08, Roads) 11.771 → 11.608 (−163, 0 slechter); C7 `p6StartedTaskIgnoresPlannedStartFloor` (B11, OZB+Roads, zonder `restart_date` te lezen) 11.608 → 11.529 (−79, 0 slechter); herpin per commit (HERPIN 2026-09-23d/e); B05 geschrapt (patch in /tmp, +72/0 rehab-2 alleen). Let op voor §1d-3: vijf lopende CP_Phys-taken in Roads komen binnen dezelfde bucket verder van P6 (oorzaak B07); telling 21 (wordt 22 na merge met C5). Merget ná brok 2 en brok 3 |
...
| X12 naar nul — brok 5: B15 (C8) + B12 ALAP (C9) + B13/B14 (C10/C11, mits verdedigbaar) | `claude/x12-brok5-klein` (basis `5fcd85de` = C7; worktree `agent-x12-brok5-klein`) | kop `030e22a6` (gepusht): **C9 `p6AlapPositionedFromSuccessors` (B12) 11.529 → 11.502** (−27, 0 slechter per emmer, HarbourPointe) — maar EC1420/EC1430 gingen van 11 naar 1.440 werkminuten en EC1030 van 43.680 naar 81.120 in dezelfde emmer ⇒ onder de grootte-ratchet ROOD; analyse klaar (`d9973123`): EC1030 opgelost (C9 alleen voor niet-gestarte ALAP op uurkalender); 3 cellen blijven groter (EC1420 es/ef, EC1430 es) door een P6-DUURregel buiten ALAP: EC1430 en EC1590 krijgen in P6 span 696 u bij `remain_drtn_hr_cnt` 720 = `remain_qty/remain_qty_per_hr` van resource 6686 (EC1810 zelfde patroon, wel 720 — onderscheid onbekend). C10-onderzoek klaar: over 5.660 niet-gestarte P6-taken wijken 9 spans af (8 HarbourPointe DT_FixedDrtn + E-1000/B10); beste formule max(qty/rate zonder toewijzingslag) laat 3 fouten (EC1430/EC2380/EC1590) — zes taken met identieke invoer (6604 vs overige) splitsen in P6 twee kanten op ⇒ niet uit invoer af te leiden, C10 NIET gebouwd. **C9 geparkeerd** (regel A per cel incl. grootte, geen uitzondering); dossier in plan XER §9 en C8-patch onder `docs/superpowers/plans/patches/` zijn overgenomen op de etappebranch (`5e0b88c8`); brok 5 is daarmee afgesloten, de branch blijft als naslag. C8 (B15) +2/−1 (rehab-2) als patch bewaard tot na de populatiewijziging; B13/B14 overgeslagen (n=1, mechanisme niet aangetoond). Merget ná brok 4 + manifest + ratchet |
| **manifest-etappe: populatie = P6-doorgerekende orakels** (besluit 23-09) | `claude/x12-manifest-p6-orakels` (kop `8cc5a108`, gepusht) | **klaar; critreview = LANDEN-MET-FIXES (getallen kloppen: geen cel veranderd, pins afleidbaar, replay-B1 correct; tekstfixes: exclusionReason per bestand eerlijk, README-regel begrenzen tot het besluit, ashspace-twijfel + B5 enkele bron vastleggen; vervolg: blast-radius volgt manifest niet, As-Built-vangnet vervangen door corpusloze casus — alles doorgegeven aan de integratie-agent)**: 45 → 13 orakelentries (9 geselecteerd, 21 projecten, 5.983 taken; rol `reader-only` + `exclusionReason`; As-Built uit als unknown; Hotel blijft in incl. CR — per project uitsluiten is vervolgpunt); **X12 11.529 → 1.274** (Roads 811, HarbourPointe 192, OZB 98, DCP-03 BL 92, Hotel 64, Sample 13, ashspace 4; drivingPath 176); geen cel in P6-bestanden veranderd. Meting per conventie op de nieuwe populatie: **B3, B4, C1, C3, C4, A17 = 0 effect** (P3-gedrag); B1 uit = +3.092 (Hotel), B2 +109, B5 +20 (echt P6). `sched_lag_early_start_flag`: Y overal behalve DCP-03 Baseline (N). Replay-pin: A17-kandidaat bewijst niets meer ⇒ B1 als negatieve controle. Vervolgpunten: As-Built-vangnet voor naïeve B07-poort vervangen door corpusloze casus; Hotel/CR per project. **Integratie-agent (gestart ~18:30) tussenstand**: brok 4 en manifest gemerged, **X12 op de P6-populatie = 428**; C1 en C4 P6=uit (0 cellen); **C3 blijft P6=aan** — niet inert: C5 rekent de lag vanaf zijn punt met de C3-regel (C3 uit = +640 in Roads/HarbourPointe), de "inert"-meting van de manifest-etappe was op de brok-4-motor zonder C5; blast-radius-pin ging omhoog uitsluitend door rehab-2 ⇒ orkestratorbesluit: de check volgt voortaan het manifest (populatie-herpin met toelichting), niet omhoog pinnen. Volgt: ratchet, review-fixes, As-Built-casus, verify |
| **grootte-ratchet** (besluit 23-09) | `claude/x12-grootte-ratchet` (kop `1677c64d`, basis `d4a66772`, gepusht) | critreview = LANDEN-MET-FIXES (v1-kant bij merge = achterdeur: proefmerge met brok 2 gaf `groter=761` op de v2-kant en stil 0 op de v1-kant; diff→sameday met meer minuten telde als verbetering; commentaar 0,5 MB) — landfixes klaar `2b3c3a97` (gepusht; v1-route alleen met `OPS_XER_CELLS_V1_UPGRADE=1`, diff→sameday met meer minuten = groter, commentaar), besluiten in §1c; wordt nu gemerged door de integratie-agent (stap 4, ná manifest): cellenbestand versie 2 `{bucket, minutes}` (minuten, ook tf/ff; null bij missing/drivingPath), ratchet "niet groter" bij gelijke emmer, categorieën `groter`/`kleiner`, status `VERBETERD (grootte)`, v1 geweigerd behalve via `CELLS_WRITE=1`; CELLDELTA 0/0/0/0/0 op 15.056; merge-instructie in README (één kant nemen, dan `CELLS_WRITE=1`). Merget ná de manifest-etappe; de brok-2-merge hoort dan `groter` te melden (761 cellen) — dat is de eerste echte toets |
