...
| X12 naar nul — brok 4: B08 FF→startmijlpaal (C6) + B05 actief-rest-0 late kant (C7) | `claude/x12-brok4-ff-mijlpaal` (basis `1aa63a40` = C4; worktree `agent-x12-brok4-ff-mijlpaal`) | **klaar, kop `5fcd85de` (gepusht), critreview = LANDEN-MET-FIXES (C6: drijvend FF→startmijlpaal verschuift ES ongemeten; voortgangstak zonder bron; "(gemeten)" bij uurpoort; C7 trace-kleinigheid) — landfixes klaar `094b0d2e` (C6 raakt de ES van de mijlpaal niet meer, voortgangstak eruit, uurpoort eerlijk, C7-trace `floorApplied`; meting gelijk 11.529) — task-replay-pin was vergeten (zelfde tweede-orde pin als bij brok 2) ⇒ agent merget nu de etappebranch in en draait het 6-stappen-recept**: C6 `p6FinishFinishStartMilestoneLateFinish` (B08, Roads) 11.771 → 11.608 (−163, 0 slechter); C7 `p6StartedTaskIgnoresPlannedStartFloor` (B11, OZB+Roads, zonder `restart_date` te lezen) 11.608 → 11.529 (−79, 0 slechter); herpin per commit (HERPIN 2026-09-23d/e); B05 geschrapt (patch in /tmp, +72/0 rehab-2 alleen). Let op voor §1d-3: vijf lopende CP_Phys-taken in Roads komen binnen dezelfde bucket verder van P6 (oorzaak B07); telling 21 (wordt 22 na merge met C5). Merget ná brok 2 en brok 3 |
