...
| **integratieronde 2** | `claude/x12-integratie-2` (basis `1b7cdebb`; worktree `agent-x12-integratie-2`) | **klaar en gemerged (`3b40aa0f` → `44639f59`)**: brok 8 → 180, tolerantie → 178, policy-fix (=corpus) 178, brok 9 → 175, C6-optie 175, uitsluitingsmechanisme 175; conflicten alleen in tellingen/docs/pins (herpind); C12-fixture (e) aangepast (C2-breed rekent FF+lag nu zelf: 28/9 gepind); C13-voltooiingspoort niet mutant-testbaar (A12 zet voltooid werk op ff 0); `verify` groen op `3b40aa0f`; tellingen zesentwintig conventies / tien projectopties |
