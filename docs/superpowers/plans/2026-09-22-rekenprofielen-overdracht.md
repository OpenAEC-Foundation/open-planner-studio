...
| parallelle sporen 24-09 ~22:30 | `claude/x12-c10-alap-port` (C10 → C14 volledig op de huidige motor, NIET landen vóór de uitsluitingen); `claude/x12-blast-default-keys` (`DEFAULT_KEYS` + `startToStartLagFrom`, v11); `claude/x12-nivellering-onderzoek` (P6-nivellering als eigen etappe: Oracle-instellingen, OZB 9033 als casus, etappevoorstel); `claude/x12-gebruikstest-26` (browser-gebruikstest van het profielblok met 26 conventies/10 opties, screenshots in `qa/`); `claude/x12-pr169-review` (critreview PR #169 als geheel + PR-body-voorstel + volledige verify) | gestart |
