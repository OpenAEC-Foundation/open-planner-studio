...
| rekenprofielen (etappe) | `claude/rekenprofielen` (bovenop de PR-branch) | kop `4536eea9` (alle banen + fixes + brok 1); eindreview I4 her-check: alle 12 punten verwerkt, laatste must-fix (Toepassen bij lege profielnaam) gemerged; finale `verify` + `measure:profiles` lopen; I5 gebruikstest gedaan (screenshots in agent-worktree `agent-a6504e1b939ec3d0a/qa/`): 2 productpunten (twee undo-stappen per wissel door `setProject`-metadata; spatie in naam — mogelijk al gefixt) + 3 gidszinnen → D-agent fixt; daarna nog één verify en I6 draft-PR gestapeld op #109 (PR-tekst: export-guard-gat .mpp→MSPDI, C5 releasenotitie, MSPDI/P6-XML openen als OPS) |
