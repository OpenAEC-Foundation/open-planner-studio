...
**25-09 20:00 — #109 GEMERGED naar `main` (`1d8f2df6`, `gh pr merge --merge --admin` na groene CI op `389ddc06`: test + 3 builds + CodeQL pass); main-CI/deploy loopt (`gh run list --branch main`).** Etappe-keten op `f058c659`: MEASURE_EXIT=0 (76/0/0/0), verify EXIT=0 (browser 173/173). Taaktypes-keten op `4ed84fbd`: MEASURE_EXIT=0, verify loopt.
