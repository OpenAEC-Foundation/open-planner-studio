...
import { createXerSourceArchive, decodeXerSourceArchive, sha256Hex } from '@/services/xerSourceArchive';
...
const archive = createXerSourceArchive(bytes, {
  encoding: 'windows-1252', bom: 'utf-8', newline: 'crlf', diagnostics: { enumFallbacks: [{ token: 'ST_TotalFloat' }] },
});
...
if (failures.length === 0) { console.log('OK  ifc-xer-archive-container: alle checks groen (11)'); process.exit(0); }
