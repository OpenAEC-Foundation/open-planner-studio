...
// discriminator werd al door de I4/T6-fixture in check-mpp-calendars.ts bewezen).
{
  ...
  // datumsignaal vuurt, een dubbele bevestiging). ──────────────────────────────────────────────
  {
    ...
    if (result) {
      ...
      truthy('uurmodus-fixture (uren-taak): expliciete taakeenheid is hours', task?.time.durationUnit === 'hours');
  ...
  // landen ⇒ geen enkel (c)-signaal ⇒ GEEN workTime-/durationMinutes-lek. ─────────────────────
  {
    ...
    if (result) {
      ...
      truthy('uurmodus-fixture (dag-contrast): expliciete taakeenheid is days', task?.time.durationUnit === 'days');
