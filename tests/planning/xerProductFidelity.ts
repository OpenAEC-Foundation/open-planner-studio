...
function validateIdentity(
  ...
  for (const task of solved?.tasks ?? []) {
    ...
    if (!task.taskCode.trim()) errors.push(`project ${projectId}/taak ${task.sourceTaskId}: taskCode is leeg`);
    if (byId.has(task.sourceTaskId)) errors.push(`project ${projectId}: dubbele opgeloste taak-id ${task.sourceTaskId}`);
    else byId.set(task.sourceTaskId, task);
  }
  const truthById = new Map(truthTasks.map(task => [task.taskId, task]));
  for (const id of [...byId.keys()].filter(id => !truthById.has(id)).sort()) errors.push(`project ${projectId}: extra taak ${id}`);
  for (const id of [...truthById.keys()].filter(id => !byId.has(id)).sort()) errors.push(`project ${projectId}: ontbrekende taak ${id}`);
  for (const task of truthTasks) {
    const actual = byId.get(task.taskId);
    if (actual?.taskCode && actual.taskCode !== task.taskCode) {
      errors.push(`project ${projectId}/taak ${task.taskId}: code verwacht ${task.taskCode}, kreeg ${actual.taskCode}`);
    }
  }
  return byId;
}
...
export function measureXerProductFidelity(
  ...
  for (const [projectId, truthTasks] of truthByProject) {
    const solvedById = validateIdentity(projectId, truthTasks, solvedByProject.get(projectId), identityErrors);
