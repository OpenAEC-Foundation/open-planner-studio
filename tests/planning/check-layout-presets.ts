...
import {
  ...
  migrateSavedFilters, pickLayoutParts, type LayoutViewParts,
...
const resourceDiagram: Layout = {
  ...
  group: [{ field: { src: 'resource' }, dir: 'asc' }], sort: [],
};
...
// --- Delen en match ---
ok(layoutParts(resourceDiagram).join(',') === 'group,sort', 'layoutParts in vaste UI-volgorde');
...
const full: Layout = { id: 'l4', name: 'Vol', columns: [], filter: null, group: [], sort: [], timeScale: 'month' };
...
await saveLayouts([resourceDiagram, full, clearFilter]);
const roundTrip = await loadLayouts();
ok(roundTrip.length === 3, 'gedeeltelijke en volledige layouts overleven een round-trip');
ok(layoutParts(roundTrip[0]!).join(',') === 'group,sort', 'een gedeeltelijke layout krijgt er bij het laden geen delen bij');
