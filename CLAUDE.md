...
- `artifacts/<onderwerp>/` — **gecommit schermbewijs** bij een PR (bijv. `artifacts/resourcediagram/`), waar een PR-tekst of issue-reactie naar linkt via `raw.githubusercontent.com`. Klein houden: PNG, ≤ ~150 KB per bestand, een handvol per PR, en na de merge niet bijwerken (het is bewijs van dat moment, geen documentatie). Lokale QA-screenshots horen in `qa/` (in `.gitignore`).
