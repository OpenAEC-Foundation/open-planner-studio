...
**Een startdatum typen op een taak met een voorganger.** De voorganger bepaalt wanneer zo'n taak kan beginnen, dus een getypte startdatum alleen zou na het herberekenen niets doen. Daarom maakt Open Planner Studio er, net als MS Project, automatisch een constraint **Start niet eerder dan (SNET)** van: in de Tabel (kolom Start of Geplande start), in het eigenschappenpaneel en in **Taak bewerken**. Een melding vertelt wat er gebeurde. Ligt de datum vóór wat de voorganger toelaat, dan wint de voorganger; ligt hij later, dan begint de taak na herberekenen (F5) op die datum. Had de taak al een SNET, dan schuift alleen de datum mee; een ander constrainttype (bijvoorbeeld ALAP of MSO) blijft staan zoals het is. Een handmatig geplande of al gestarte taak krijgt geen constraint: daar bepaalt de ingevoerde of de werkelijke start de planning al. **Ctrl+Z** maakt de startdatum en de constraint samen ongedaan.
