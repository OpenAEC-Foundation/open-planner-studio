...
Dubbelklik een taak en kijk in de sectie **CPM Resultaat** voor de exacte cijfers: **Vroegste start**, **Vroegste einde**, **Laatste start**, **Laatste einde**, **Totale speling**, **Vrije speling** en (indien van toepassing) **Interfererende speling**, plus of de taak op het **Kritiek pad** ligt. Wil je deze velden als kolommen in de taaktabel? Klik op het plusje rechts in de tabelkop en kies ze in de kolomkiezer onder **Berekend**, telkens één; een veld dat al een kolom is, staat aangevinkt. Zie [Kolommen kiezen](docs://ref-kolommen).
...
Grote projecten bestaan soms uit meerdere, apart beheerde deelplanningen — bijvoorbeeld je eigen hoofdplanning en een terreininrichting die een andere aannemer beheert. Een **externe koppeling** legt een relatie naar een taak in zo'n ander bestand vast zonder dat bestand als document te openen. Selecteer één taak en kies **Relatie → Externe relatie toevoegen** op het tabblad **Start**, **Planning** of **Tabel** om het venster **Externe (cross-project) koppeling** te openen.
...
Wil je weten of het bronbestand intussen is bijgewerkt? Rechtsklik het externe token in de kolom **Voorgangers** of **Opvolgers** en kies **Bron vernieuwen**, of kies **Relatie → Alle externe relaties vernieuwen** in het lint om alle bronnen opnieuw in te lezen. Is het bronbestand niet beschikbaar — verplaatst, hernoemd, of nooit meegeleverd — dan toont de koppeling het label **verouderd** met de tooltip "bron niet geladen — her-importeer om te verversen": de app kan dan niet zelf verifiëren of het bevroren anker nog klopt.
