...
- **Start date** — shows the computed early start; a manual change anchors the new date as the planned start. If the task has a predecessor, **Save** also turns that date into a Start no earlier than (SNET) constraint, unless you pick a constraint yourself in the same dialog.
