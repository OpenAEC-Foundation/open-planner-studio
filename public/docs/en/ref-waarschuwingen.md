...
- Click one of the yellow counters in the status bar — for example **⚠ 2 deadline(s) missed**.
- Or use the ribbon: **View → Panels → Warnings**, or **Planning → Schedule → Warnings**, next to **Calculate**.
- The panel appears at the bottom of the right-hand sidebar, below Properties and the Resource dock. Drag the edge above it to change its height; the height is remembered.
...
- **Relation ignored** — the relation does not touch a leaf task and is not part of the calculation.
...
On the **Table** tab the task is selected in the table; the Gantt makes the jump as soon as you switch back.
