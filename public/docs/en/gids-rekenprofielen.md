...
## The twenty-two conventions

Under Open Planner Studio all twenty-two are off.
...
- **ALAP task as late as its successors allow** (Primavera P6) — an unstarted task with the *as late as possible* constraint finishes exactly where its successors allow, to the minute; a chain of such tasks closes up. Its own planned start does not count, not even as a floor. The successors do not move. With the convention off, the task moves by its free float in whole working days.
...
The profile is saved in the IFC file, with all twenty-two values, so the file calculates the same everywhere. A project with the default profile saves nothing extra. Older versions of Open Planner Studio do not know the profile: they only read the calculation options and the two progress conventions of Microsoft Project, and calculate a P6 project without P6 conventions.
