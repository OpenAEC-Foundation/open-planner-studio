...
- **Late finish on the task's own calendar** (Primavera P6) — if a task's late finish comes from a successor that uses another calendar, and that boundary falls outside the task's own working time, the late finish becomes the end of the previous work period on the task's own calendar. Example: a task does not work on Fridays and its successor must start on Friday at 16:00; its late finish is then Thursday 17:00.
