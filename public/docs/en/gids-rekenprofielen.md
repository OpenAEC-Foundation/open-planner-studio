...
- **Free float in the task's own calendar** (Primavera P6) — the free float of an open task counts per relationship in the task's own calendar, not in the successor's. This holds for every relationship type and also with a working-time lag, as long as that lag runs on the predecessor's calendar. Example: a task works until 17:00, its successor (start-to-start with lag) until 16:00. If the relationship boundary falls at 16:00, the successor only starts the next morning and the task has one hour of free float. Elapsed-time lags, percentage lags and lags on another lag calendar have not been measured and keep the ordinary calculation.
