...
- Möchten Sie diese Daten auch als Spalten in der Aufgabentabelle haben, statt jede Aufgabe öffnen zu müssen? Auf der Registerkarte **Tabelle** stehen **Kritisch** und **Gesamtpuffer** standardmäßig schon in der Tabelle. In der Aufgabenliste neben dem Gantt fügen Sie sie mit dem Plus rechts im Tabellenkopf hinzu: Es öffnet die Spaltenauswahl, in der Sie unter **Berechnet** jeweils ein Feld wählen. Siehe [Spalten wählen](docs://ref-kolommen).
...
- Sie wissen jetzt, wie die Schaltflächen funktionieren; wenn Sie wissen wollen, was einen Terminplan verlässlich macht — Granularität, Verknüpfungen, Einschränkungen, Puffer, Baseline —, lesen Sie die Anleitung [Gut planen](docs://gids-goed-plannen).
