...
Quand aucune courbe ne convient, fixez vous-même les heures par jour ouvré. À côté de la liste des courbes de chaque affectation se trouve le bouton **Répartition des heures…** (l'icône en barres) ; il ouvre une fenêtre avec une ligne par jour ouvré de la tâche — les mêmes jours que ceux sur lesquels l'histogramme comptabilise, donc en sautant les jours de pause d'une tâche fractionnée. Chaque ligne montre la date, le travail **restant** en heures (modifiable) et les unités dérivées ; si l'affectation porte déjà du travail **réalisé** (import avec avancement), il apparaît dans une colonne à part, en lecture seule. Le point de départ est exactement ce que l'affectation comptabilise aujourd'hui : la courbe choisie, ou la courbe exacte issue d'un import P6 ou MS Project.

- **Appliquer une forme** remplit les lignes avec l'une des huit formes standard (les six courbes plus *Double pic* et *Tortue* de MS Project et P6), en conservant le total actuel — un point de départ pratique à ajuster ensuite jour par jour.
- **Appliquer** enregistre la répartition comme *contour* de cette affectation. La liste des courbes affiche alors **Contour** et est désactivée : le contour est une donnée et l'emporte sur toute courbe. Histogramme, surallocation, lissage et vue d'occupation l'utilisent immédiatement, et la répartition voyage dans le fichier IFC et dans un export MS Project XML ou Primavera P6 XML.
- **Libérer la répartition** supprime le contour ; l'affectation suit ensuite de nouveau sa courbe.

Un contour ne change que les heures par jour de cette seule affectation. Les dates de la tâche, sa durée et ses éventuelles interruptions restent telles quelles — un jour à 0 heure reste aussi dans la durée de la tâche et ne crée pas d'interruption. Pour raccourcir, allonger ou interrompre la tâche elle-même, modifiez la tâche. Si vous changez la durée plus tard, le contour s'étire ou se contracte proportionnellement (voir le guide [Import MS Project](docs://gids-msproject-import), section Affectations profilées). Le panneau des propriétés marque une tâche ayant ses propres répartitions d'heures par un badge gris renvoyant à cette même section. Appliquer et libérer sont des étapes d'annulation ordinaires.
